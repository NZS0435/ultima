...
4. The secure mailbox path now uses the standard `AES-256-GCM` primitive, but production-grade key management, secret storage, and external operational review are still absent.
...
3. Add real key management, secret storage, and operational controls if production-grade claims are ever desired.
